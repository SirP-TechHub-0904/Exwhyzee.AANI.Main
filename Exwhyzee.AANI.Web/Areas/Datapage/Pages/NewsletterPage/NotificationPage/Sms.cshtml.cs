...
namespace Exwhyzee.AANI.Web.Areas.Datapage.Pages.NewsletterPage.NotificationPage
{
    public class SmsModel : PageModel
    {
        ...
        public SmsModel(AaniDbContext context, UserManager<Participant> userManager, IRecipientParser recipientParser, ITemplateRenderer templateRenderer)
        {
            _context = context;
        ...
        public async Task OnGetAsync()
        {
            TemplateList = await _context.MessageTemplates
                .Where(t => t.IsActive && t.MessageType == MessageType.Sms)
        ...
        public async Task<IActionResult> OnGetParticipantsAsync(string source, long chapterId = 0, long secId = 0, bool preview = false)
        {
            IQueryable<Participant> q = _userManager.Users.Include(x => x.Chapter).Include(x => x.SEC)
                // keep same MNI filter; exclude internal addresses if desired (copied from original)
        ...
        private static string NormalizePhone(string? phone)
        {
            ...
            // optionally remove leading country code zeroes or keep as-is; keep digits-only version
            return digits;
        }
